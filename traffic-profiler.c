...
struct ip_hdr_prof hdr_list[1000],*ip_hdr_ptr;
...
/***************************************************************/
       ...
       for(i=0;i<hdr_list_count;i++){
	       ip_hdr_ptr=&hdr_list[i];
	       if(ip_hdr_ptr->ip_addr[0]==p1 && ip_hdr_ptr->ip_addr[1]==p2 && ip_hdr_ptr->ip_addr[2]==p3 && ip_hdr_ptr->ip_addr[3]==p4 && ip_hdr_ptr->port_num==port_d){
		       ip_hdr_ptr->counter++;		
               printf("Remote IP:%u.%u.%u.%u\tRemote Port:%u\nMatch Count:%u\n",(ip_hdr_ptr->ip_addr[0]),ip_hdr_ptr->ip_addr[1],ip_hdr_ptr->ip_addr[2],ip_hdr_ptr->ip_addr[3],ip_hdr_ptr->port_num,ip_hdr_ptr->counter);
		       break;
	       }
       }
       if(hdr_list_count==0 || (!(hdr_list_count==0) && i==hdr_list_count)){
	       ip_hdr_ptr=&hdr_list[hdr_list_count];
	       ip_hdr_ptr->ip_addr[0]=p1; 
	       ip_hdr_ptr->ip_addr[1]=p2;
	       ip_hdr_ptr->ip_addr[2]=p3;
	       ip_hdr_ptr->ip_addr[3]=p4;
	       ip_hdr_ptr->port_num=port_d;
	       ip_hdr_ptr->counter=1;
	       hdr_list_count++;
